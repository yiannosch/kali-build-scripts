...
###-- Update hostname to preset value --###
# If default(kali) is selected then skip
(( STAGE++ ))
...
apt -qq update && apt install -y -q atom
...
###-- Initialise metasploit detabase --###
(( STAGE++ ))
...
# Add postgreSQL service to startup
...
if [ -s $file ]; then
  ...
  # Modify the installer to shut up
